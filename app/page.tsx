...
export default function Home() {
  ...
  const [aiView, setAiView] =
    useState<"none" | "restaurants" | "menu">("none");
  ...
  const handleAIResponse = (data: any) => {
    ...
    if (data.intent === "show_restaurants") {
      ...
      if (data.filters?.spicy) {
        const spicy = restaurants.filter(
          (r) =>
        ...
        const transformedRestaurants: Restaurant[] = data.map(
          (r: OSMRestaurant, idx: number) => {
            return {
              ...
              image:
                r.image ||
                `https://picsum.photos/seed/restaurant-${idx + 1}/900/400`,
            };
          }
        );
  ...
  const handleCheckout = async () => {
    ...
    await saveOrder(String(id), deliveryAddress);
          ...
          <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                {user
                  ? `Welcome, ${user.name}`
                  : "Food delivery in your neighborhood"}
              </p>
            </div>
        ...
        <main className="flex h-[calc(100vh-100px)]">
          <aside className="w-64 border-r p-4 hidden md:block">
            ...
            <div className="space-y-2 text-sm">
              <p className="p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/10">
                Past Orders
              </p>
              <p className="p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/10">
                Saved Meals
